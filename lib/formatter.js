...
function formatImage(elem, options) {
  ...
  // add support arbitrary data
  // support especially article as posted on wechat platform
	if (attribs['data-type'] == 'png' ||
		  attribs['data-type'] == 'jpg' ||
		  attribs['data-type'] == 'gif' ||
		  attribs['data-type'] == 'tiff' ||
		  attribs['data-type'] == 'jpeg') {
		if (attribs['data-src']) {
			result += '[' + attribs['data-src'] + ']';
		}
	}
